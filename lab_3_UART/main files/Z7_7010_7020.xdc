...
##Buttons - btn[0]=rst, btn[1]=send trigger
set_property -dict { PACKAGE_PIN K18   IOSTANDARD LVCMOS33 } [get_ports { btn[0] }]; #IO_L12N_T1_MRCC_35 Sch=btn[0] - RST
set_property -dict { PACKAGE_PIN P16   IOSTANDARD LVCMOS33 } [get_ports { btn[1] }]; #IO_L24N_T3_34 Sch=btn[1] - SEND
...
##Pmod Header JC - USB-UART Adapter (Zybo Z7)
## Adapter pinout: Pin1=RTS, Pin2=RXD, Pin3=TXD, Pin4=CTS
## JC1(V15)=RTS, JC2(W15)=RXD(adapter receives from FPGA TX), JC3(T11)=TXD(adapter sends to FPGA RX), JC4(T10)=CTS
set_property -dict { PACKAGE_PIN V15   IOSTANDARD LVCMOS33     } [get_ports { RTS }]; #IO_L10P_T1_34 Sch=jc_p[1] - RTS (directly grounded in VHDL)
set_property -dict { PACKAGE_PIN W15   IOSTANDARD LVCMOS33     } [get_ports { RXD }]; #IO_L10N_T1_34 Sch=jc_n[1] - RXD (FPGA TX -> adapter RX)
set_property -dict { PACKAGE_PIN T11   IOSTANDARD LVCMOS33     } [get_ports { TXD }]; #IO_L1P_T0_34 Sch=jc_p[2] - TXD (adapter TX -> FPGA RX)
set_property -dict { PACKAGE_PIN T10   IOSTANDARD LVCMOS33     } [get_ports { CTS }]; #IO_L1N_T0_34 Sch=jc_n[2] - CTS (directly grounded in VHDL)
